...
| **Región/comunidad por proyecto** + asignar zonas enteras (N:N) | 🧪 | Etiqueta `región` por ciudad; en Permisos las casillas se agrupan por región y se marca una comunidad entera de golpe (SICE: C.Valenciana+Murcia). No toca el aislamiento (sigue por `project_id`) |
