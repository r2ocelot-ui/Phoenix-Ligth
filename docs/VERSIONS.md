...
| **Emergencia por CM + modal de confirmación con lista** | 🧪 | El botón "🚨 Emergencia" (y su contraparte "✓ Apagar emergencia") ya no abren un `confirm()` con solo el contador: ahora abren un modal con la **lista de cuadros afectados** y checkboxes para destildar los que no quieras forzar. Si están todos marcados, se manda como antes (todo el scope); con selección parcial, body `{cabinet_codes:[…]}`. Backend valida intersección con la tenencia (no se puede saltar el scope listando códigos de otra ciudad). Test del scope parcial nuevo |
